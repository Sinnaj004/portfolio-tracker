...
def get_dashboard_summary(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    for portfolio in portfolios:
        ...
        summary_list.append(PortfolioSummary(
            id=portfolio.id,
            name=portfolio.name,
            description=portfolio.description,  # <--- Hinzugefügt
            ...
            profit_loss_pct=float(profit_pct.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)),
            # float für JSON-Kompatibilität falls nötig
